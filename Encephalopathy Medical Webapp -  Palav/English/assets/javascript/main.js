...
class QualifyingSection {
    ...
    _reasons(num) {
        if (num == 1) {
            return "The infant's gestational age is less than 36 weeks."
        } else if (num == 2) {
            return "The infant is greater than 6 hours old."
        } else if (num == 3) {
            return "The infants birth weight is less than 1800g."
        } else if (num == 4) {
            return "The infant suffers from congenital abnormalities."
        } else if (num == 5) {
            return "The infant suffers from chromosomal abnormalities."
        } else if (num == 6) {
            return "There is an alternate cause for encephalopathy."
        }
    }
}
/**
...
class ResultSection {
    ...
    createSummary() {
        ...
        if (values.s1q1 == true) {
            // Siezures questions
            if (values.s2q1) {
                summary = "Patient presents with seizures."
            } else { // Sarnat 
                // Normal Sarnat
                if (values.s3q2 == 0) {
                    summary = "Patient's neurological exam is normal according to Sarnat staging."
                }
                // Mild sarnat
                else if (values.s3q2 < 3) {
                    summary = "Patient meets criteria for mild encephalopathy according to Sarnat staging."
                }
                // Severe sarnat
                else {
                    summary = "Patient meets criteria for moderate/severe encephalopathy according to Sarnat staging."
                }
            }
            ...
            // History section
            if (values.s6q1) {
                summary += ` There is history of an acute event - ${values.s6q2}.`
            } else if (values.s6q1 == false && values.s6q1 != undefined) {
                summary += " There is no history of an acute event."
            }
            if (values.s6q3) {
                summary += ` The 10 minute Apgar score is ${values.s6q3}, and the patient ${values.s6q4 ? "was" : "was not mechanically"} ventilated from birth for at least 10 minutes.`
            }
        } else {
