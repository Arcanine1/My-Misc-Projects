...
while(500>x):
    ...
    page = requests.get(URL)
    code = str(page.text)
    start= code.find("a href", start+1)
    end= code.find('"', start+10)
    print(code[start+19:end])
    match_code=code[start+19:end]

    URL = "https://www.thebluealliance.com/event/2021"+str(match_code)
    page = requests.get(URL)
    code = str(page.text)
    if(code.find("Quals")==-1):
        continue
    ...
    def get_stuff(URL, m, just_teams_array, just_auto_line_array, just_scores_array, just_climb_array,
                  just_fouls_arrays, \
    ...
    index = 0
    while (l < 100000):
        index = code.find(str(year) + match_code + "_qm" + str(l), index + 1)
        if (index == -1):
            break
        l = l + 1
    ...
    m = 0
    URL = "https://www.thebluealliance.com/match/" +str(year) + match_code + "_qm"
    while l < big_loop_parameter:
        ...
        generator_level_points = get_stuff(URL + str(l), m, just_teams_array, just_auto_line_array, just_scores_array, \
                                           just_climb_array, just_fouls_arrays, auto_lower_array, auto_outer_array,
                                           auto_inner_array, teleop_lower_array, \
                                           teleop_outer_array, teleop_inner_array, control_panel_points,
                                           generator_level_points)
    ...
    b = 0
    URL = "https://www.thebluealliance.com/match/" +str(year) + match_code + "_"
    while playoff_matches_array[b] != None:
        ...
        generator_level_points = get_stuff(URL + str(match_type_adder) + str(round_adder) + "m" + str(match_adder), m,
                                            just_teams_array, just_auto_line_array, just_scores_array, just_climb_array,
                                            just_fouls_arrays, auto_lower_array, \
                                            auto_outer_array, auto_inner_array, teleop_lower_array, teleop_outer_array,
                                            teleop_inner_array, control_panel_points, \
                                            generator_level_points)
        b = b + 3
        m = m + 1
    ...
    path = '/Users/ayushgarg/Desktop/Big Data.xlsx'
